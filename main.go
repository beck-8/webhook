...
func main() {
	...
	flag.IntVar(&maxSize, "maxSize", 1024*5, "MaxSize is the maximum size in megabytes of the log file before it gets rotated. It defaults to 1024*5 megabytes.")
	flag.IntVar(&maxBackups, "maxBackups", 5, "MaxBackups is the maximum number of old log files to retain.")
	flag.IntVar(&maxAge, "maxAge", 30, "MaxAge is the maximum number of days to retain old log files based on the timestamp encoded in their filename. Note that a day is defined as 24 hours and may not exactly correspond to calendar days due to daylight savings, leap seconds, etc.")
